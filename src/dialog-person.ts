...
import svanUrl from "../assets/svan.png";
...
export function preloadPeople(scene: Phaser.Scene) {
  ...
  scene.load.spritesheet("silkeshäger", silkäsHägerUrl, {
    frameWidth: 200,
    frameHeight: 200,
  });
  scene.load.spritesheet("tajga", tajgaUrl, {
    frameWidth: 200,
    frameHeight: 200,
  });
  scene.load.spritesheet("biatare", biatareUrl, {
    frameWidth: 200,
    frameHeight: 200,
  });
  scene.load.image("svan", svanUrl);
...
export type BirdNames = "silkeshäger" | "biatare" | "tajga" | "svan";
export type Names = "oskar" | "molly" | "adam" | "klara" | "spegel" | BirdNames;
...
export function createPerson(
  ...
  const x =
    enterAction?.from === "left"
      ? -100
      : enterAction?.from === "right"
      ? 900
      : defX;
  const y =
    enterAction?.from === "top"
      ? -100
      : enterAction?.from === "bottom"
      ? 900
      : defY;
